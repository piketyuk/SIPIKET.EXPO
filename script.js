...
function attachClassHandlers() {
  classBoxes.forEach((b, i) => {
    b.addEventListener("input", () => {
      ...
      if (teacherMode) b.value = b.value.replace(/[^A-Za-z]/g, "").slice(-1).toLowerCase();
...
const classToggle = $("#classToggle"), codeFieldset = $("#codeFieldset"), classOffHint = $("#classOffHint"), accountHint = $("#accountHint"), googleStep = $("#googleStep"), emailPreviewTo = $("#emailPreviewTo"), emailVerifyBtn = $("#emailVerifyBtn"), resendEmailBtn = $("#resendEmailBtn"), resendStatus = $("#resendStatus"), termsLink = $("#termsLink");
...
async function countSavedAccounts() {
  let n = 0;
  for (let i = 0; i < localStorage.length; i++) { const k = localStorage.key(i); if (k && k.startsWith("enc_account_")) n++; }
  return n;
}
async function showAccountHint() {
  ...
  if (n > 0) {
    ...
    document.getElementById("hintReuse")?.addEventListener("click", (e) => { e.preventDefault(); updateClassEnabled(false); document.getElementById("googleStep")?.scrollIntoView({ behavior: "smooth", block: "center" }); });
...
(function autoRestoreSession() {
  if (!sessionStorage.getItem("sipiket_registered")) {
    ...
    if (last) {
      ...
      if (raw) {
        try { const acc = JSON.parse(atob(JSON.parse(raw).ct ? "" : "")); } catch {}
      }
    }
  }
  const last = localStorage.getItem("sipiket_last_email");
        ...
        if (parsed && parsed.ct && typeof secureGet === "function") {
          secureGet("account_" + last.toLowerCase()).then((acc) => {
            if (acc && acc.email) {
              ...
              sessionStorage.setItem("sipiket_last_account_hint", JSON.stringify(acc));
              showAccountHint();
            }
          });
        }
...
function updateClassEnabled(on) {
  ...
  if (verifyBtn) { verifyBtn.style.display = on && !teacherMode ? "" : "none"; verifyBtn.disabled = !on ? true : classVerified ? true : false; }
...
function updateGoogleVisibility() {
  ...
  if (teacherMode) { googleStep.hidden = !classVerified; googleStep.style.display = classVerified ? "grid" : "none"; }
  else if (!classEnabled) { googleStep.hidden = false; googleStep.style.display = "grid"; }
  else { googleStep.hidden = !classVerified; googleStep.style.display = classVerified ? "grid" : "none"; }
}
function applyTeacherMode(on) {
  ...
  if (teacherToggle) {
    ...
    teacherToggle.setAttribute("aria-label", on ? "Kode guru aktif — klik untuk kembali ke kode kelas" : "Aktifkan kode guru");
    teacherToggle.title = on ? "Kode guru aktif" : "Kode guru";
  }
  if (classToggle) { classToggle.style.display = on ? "none" : ""; if (on && !classEnabled) updateClassEnabled(true); }
  ...
  if (loginDesc) loginDesc.textContent = on ? "Kode untuk guru, Google untuk akun. Belum daftar? Otomatis didaftarkan." : classEnabled ? "Kode kelas untuk masuk kelas, Google untuk akun. Belum terdaftar? Otomatis didaftarkan." : "Tanpa kode — Google akan auto ke kelas kamu.";
...
teacherToggle?.addEventListener("click", () => {
  ...
  otpStatus.textContent = next ? "Mode kode guru aktif" : "Mode kode kelas aktif";
...
classToggle?.addEventListener("click", () => {
  ...
  if (classVerified) resetClass();
  if (!next) { sessionStorage.removeItem("sipiket_classCode"); otpStatus.textContent = "Kode kelas dimatikan — akun lama akan auto ke kelas."; otpStatus.style.color = "var(--muted)"; }
  else { otpStatus.textContent = "Kode kelas diaktifkan"; otpStatus.style.color = "var(--muted)"; classBoxes[0]?.focus(); }
});
classToggle?.addEventListener("keydown", (e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); classToggle.click(); } });
...
function syncGoogle() {
  ...
  if (needsCode && !classVerified) googleBtn.title = teacherMode ? "Verifikasi kode guru dulu" : "Verifikasi kode kelas dulu";
...
otpForm?.addEventListener("submit", (e) => {
  e.preventDefault();
  if (!classEnabled && !teacherMode) { setClassVerified(true); return; }
  if (teacherMode) {
...
window.onGoogleCredential = async function (response) {
  try {
    ...
    if (!classEnabled && !classCodeToUse) {
      otpStatus.textContent = "Akun baru tanpa kode — silakan aktifkan Kode kelas dan masukkan kode kelas dulu.";
      otpStatus.style.color = "#d93025";
      updateClassEnabled(true);
      return;
    }
    ...
    if (emailVerifyBtn) emailVerifyBtn.href = `otp.html?email=${encodeURIComponent(email)}&t=${Date.now()}`;
    ...
    console.log("%c[Email preview — kirim via backend saat production]", "color:#ff6b00;font-weight:bold", mailHtml);
  } catch (err) {
...
function triggerGoogleChooser() {
  ...
  if (!classVerified && (teacherMode || classEnabled)) { otpStatus.textContent = teacherMode ? "Verifikasi kode guru dulu!" : "Verifikasi kode kelas dulu!"; otpStatus.style.color = "#d93025"; classBoxes[0]?.focus(); return; }
  if (!termsCheck.checked) { termsHint.textContent = "Wajib centang Syarat & Ketentuan dulu!"; termsCheck.focus(); return; }
  const demoEmail = "siswa" + Math.floor(Math.random() * 900 + 100) + "@gmail.com";
  const fakeCred = btoa(JSON.stringify({ email: demoEmail, name: "Siswa Demo", picture: "" })).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
  window.onGoogleCredential({ credential: "eyJhbGciOiJIUzI1NiJ9." + fakeCred + ".sig" });
}
...
googleBtn?.addEventListener("click", (e) => {
  ...
  if (googleBtn.disabled) {
    ...
    else if (!classVerified && (teacherMode || classEnabled)) otpStatus.textContent = teacherMode ? "Verifikasi kode guru dulu!" : "Verifikasi kode kelas dulu!";
    return;
  }
...
resendEmailBtn?.addEventListener("click", async () => {
  ...
  if (resendStatus) { resendStatus.textContent = "Terkirim! Cek email (demo OTP: " + newOtp + ")"; resendStatus.style.color = "var(--orange)"; }
  if (emailVerifyBtn) emailVerifyBtn.href = `otp.html?email=${encodeURIComponent(email)}&t=${Date.now()}`;
});
...
emailForm?.addEventListener("submit", async (e) => {
  ...
  if (sessionStorage.getItem("sipiket_pendingRole") === "guru") sessionStorage.setItem("sipiket_role", "guru");
  emailStatus.textContent = "✓ Terverifikasi — konfirmasi terakhir, membuka pengisian profil...";
  emailStatus.style.color = "var(--orange)";
...
(function kelasBoot() {
  ...
  const features = [
    { icon: "⚖️", title: "Pembagian Tugas", desc: "Di dalam regu — otomatis adil / manual oleh guru", badge: "Regu" },
    { icon: "🎥", title: "Video Bukti Piket", desc: "Di dalam regu — rekam beberapa detik + keliling kelas", badge: "Regu" },
    { icon: "📋", title: "Tugas Harian", desc: "Di dalam regu — daftar tugas & status selesai", badge: "Regu" },
  ];
  const targetGrid = reguGrid || guruGrid;
  if (targetGrid) {
    targetGrid.innerHTML = features.map((f) => `<article class="card reveal in"><div class="icon">${f.icon}</div><h3>${f.title}</h3><p>${f.desc}</p><span class="badge">${f.badge}</span></article>`).join("");
  }
  ...
  notifToggle?.addEventListener("change", async () => {
    if (notifToggle.checked && "Notification" in window) {
      ...
      notifStatus.textContent = perm === "granted" ? "✓ Notifikasi Senin–Jumat 17:00 aktif (terenkripsi lokal)" : "Izin notifikasi ditolak";
    } else notifStatus.textContent = notifToggle.checked ? "Notifikasi aktif (demo lokal)" : "Notifikasi dimatikan";
